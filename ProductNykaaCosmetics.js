...
async function GETCosNykaaProducts() {
  ...
  let res = await fetch(`https://642700c9d24d7e0de47da0bc.mockapi.io/products`)
...
data.forEach((prod) => {
  ...
  pname.textContent = prod.product_name
  ...
  mrp_price.textContent = mrp.textContent + prod.product_price

  let rati = document.createElement("p")
  rati.setAttribute("class" , "starating")
  rati.textContent = prod.product_rating
